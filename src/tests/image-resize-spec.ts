...
describe('Test image resize utility functions', () => {
  it('resizeImage with source image file that exists', async () => {
  ...
  it('resizeImage with source image file that does not exist', async () => {
    resizeImage(IMAGE_FILE_NON_EXISTENT, WIDTH, HEIGHT).catch((error) => {
      expect(error).toBeInstanceOf(Error);
      expect(error.message).toContain('Input file is missing');
    });
  });

  it('resizeImage with source image file of unsupported format', async () => {
    resizeImage(IMAGE_FILE_UNSUPPORTED_FORMAT, WIDTH, HEIGHT).catch((error) => {
      expect(error).toBeInstanceOf(Error);
